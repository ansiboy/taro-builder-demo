...
import { DataForm, TextInput, rules, DataSwitch, RegionSelector } from "../../data-controls/index";
...
export default class ReceiptEditPage extends React.Component<Props, State> {

    private inputs: { [name in keyof ReceiptInfo]: React.Component<AtInputProps> } = {} as any;
    ...
    constructor(props) {
        ...
        if (params.id) {
            Taro.setNavigationBarTitle({ title: "编辑地址" });
            services.shopping.receiptInfo(params.id).then(r => {
                this.setState({ receiptInfo: r });
            })
        }
    }
